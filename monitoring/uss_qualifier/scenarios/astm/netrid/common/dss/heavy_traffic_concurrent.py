...
from monitoring.uss_qualifier.scenarios.astm.netrid.common.dss.utils import ISAValidator
...
class HeavyTrafficConcurrent(GenericTestScenario):
    ...
    def __init__(
        ...
        # TODO confirm that the auth-adapter can be reused as-is (this seems to work but a confirmation would be good)
    ...
    def _delete_isas_if_exists(self):
        ...
        for isa_id in self._isa_ids:
            self._delete_isa_if_exists(isa_id)

    def _delete_isa_if_exists(self, isa_id):
        fetched = fetch.isa(
            isa_id,
        ...
        with self.check("Successful ISA query", [self._dss.participant_id]) as check:
            if not fetched.success and fetched.status_code != 404:
                check.record_failed(
                    ...
                    Severity.High,
                    f"{self._dss.participant_id} DSS instance returned {fetched.status_code} when queried for ISA {isa_id}",
            ...
            with self.check(
                ...
                if not deleted.dss_query.success:
                    check.record_failed(
                        ...
                        Severity.High,
                        f"Attempting to delete ISA {isa_id} from the {self._dss.participant_id} DSS returned error {deleted.dss_query.status_code}",
