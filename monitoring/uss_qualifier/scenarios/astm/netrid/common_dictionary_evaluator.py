...
class RIDCommonDictionaryEvaluator(object):
    ...
    def _evaluate_timestamp(self, **generic_kwargs):
        ...
        def value_validator(val: str) -> StringBasedDateTime:
        ...
        def value_comparator(
            ...
            return (
                abs(v1.datetime - v2.datetime).total_seconds() < 1
            )  # TODO: Not a constant right now, shoud it be?
    ...
    def _evaluate_track(self, **generic_kwargs):
        ...
        def value_validator(val: float) -> float:
            ...
            if val < constants.MinTrackDirection:
                raise ValueError(
                    f"Track direction is less than {constants.MinTrackDirection}"
                )
            if val > constants.MaxTrackDirection:
                raise ValueError(
                    f"Track direction is greater than {constants.MaxTrackDirection}"
                )
        ...
        def value_comparator(v1: Optional[float], v2: Optional[float]) -> bool:
            ...
            return abs(v1 % 360 - v2 % 360) < constants.MinTrackDirectionResolution
