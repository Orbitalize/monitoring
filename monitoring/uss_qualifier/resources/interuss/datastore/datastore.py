...
class CockroachDBNode(DatastoreDBNode):
    ...
    def _runs_in_secure_mode(self) -> tuple[bool, Exception | None]:
        """
        This is detected by attempting to establish a connection with the node
    ...
    def no_ssl_rejected(self) -> tuple[bool, Exception | None]:
        return self._runs_in_secure_mode()
...
class YugabyteDBNode(DatastoreDBNode):
    ...
    def runs_in_secure_mode(self) -> tuple[bool, Exception | None]:
        ...
        """
        secure_without_ssl, e_without_ssl = self._attempt_insecure_request(with_ssl=False)
    ...
    def _attempt_insecure_request(self, with_ssl: bool) -> tuple[bool, Exception | None]:
        ...
        sock.close()
        return False, Exception(f"Received RPC response from node, hex: {bytes.hex(msg_bytes)}")
